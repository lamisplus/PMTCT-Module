...
public interface InfantRapidTestRepository  extends CommonJpaRepository<InfantRapidAntiBodyTest, String>{
    ...
    @Query(value = "SELECT unique_uuid FROM public.pmtct_infant_visit \n" +
            "WHERE infant_hospital_number = ?1 and archived = false and visit_date =(  \n" +
            "\t\t SELECT  MAX(visit_date) FROM public.pmtct_infant_visit  WHERE infant_hospital_number = ?1 and mother_patient_uuid = CAST(?2 AS VARCHAR) and archived = false \t\n" +
            ")", nativeQuery = true)
    String getLastInfantVisit (String infantHospitalNumber, String motherUuid);

    @Query(value = "SELECT unique_uuid FROM public.pmtct_infant_visit " +
            "WHERE infant_hospital_number = ?1 AND mother_patient_uuid = CAST(?2 AS VARCHAR) AND pmtct_cycle_uuid = ?3 AND archived = false " +
    ...
    @Query(value = "SELECT * FROM public.pmtct_infant_rapid_antibody WHERE unique_uuid = CAST(?1 AS VARCHAR) AND archived = false ORDER BY visit_date DESC LIMIT 1", nativeQuery = true)
    InfantRapidAntiBodyTest getLastInfantRapid (String lastVistUuid);

    @Query(value = "SELECT * FROM public.pmtct_infant_rapid_antibody WHERE mother_patient_uuid = CAST(?1 AS VARCHAR) AND date_of_test = ?2 AND archived = false ORDER BY id DESC LIMIT 1", nativeQuery = true)
    InfantRapidAntiBodyTest getLatestByMotherPatientUuidAndDateOfTest(String motherPatientUuid, LocalDate dateOfTest);

    @Query(value = "SELECT * FROM public.pmtct_infant_rapid_antibody WHERE unique_uuid = CAST(?1 AS VARCHAR) AND mother_patient_uuid = CAST(?2 AS VARCHAR) AND archived = false ORDER BY id DESC LIMIT 1", nativeQuery = true)
