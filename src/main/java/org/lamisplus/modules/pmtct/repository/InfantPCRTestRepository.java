...
public  interface InfantPCRTestRepository  extends CommonJpaRepository<InfantPCRTest, Long>
{
    ...
    @Query(value = "SELECT * FROM public.pmtct_infant_pcr WHERE infant_hospital_number = ?1 AND (archived = 0 OR archived IS NULL) ORDER BY id DESC", nativeQuery = true)
    ...
    @Query(value = "SELECT * FROM public.pmtct_infant_pcr \n" +
            "            WHERE infant_hospital_number = ?1 AND (archived = 0 OR archived IS NULL) ORDER BY id DESC LIMIT 1", nativeQuery = true)
    ...
    @Query(value = "SELECT * FROM pmtct_infant_pcr WHERE visit_date =?2 and infant_hospital_number=?1 AND (archived = 0 OR archived IS NULL) ORDER BY id DESC LIMIT 1", nativeQuery = true)
    ...
    @Query(value = "SELECT * FROM public.pmtct_infant_pcr WHERE unique_uuid = CAST(?1 AS VARCHAR) AND infant_hospital_number = ?2 AND visit_date = ?3 AND (archived = 0 OR archived IS NULL) ORDER BY id DESC LIMIT 1", nativeQuery = true)
