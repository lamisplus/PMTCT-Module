...
public  interface InfantPCRTestRepository  extends CommonJpaRepository<InfantPCRTest, String>
{
    ...
    @Query(value = "SELECT * FROM public.pmtct_infant_pcr WHERE infant_hospital_number = ?1 AND archived = false ORDER BY id DESC", nativeQuery = true)
    ...
    @Query(value = "SELECT * FROM public.pmtct_infant_pcr \n" +
            "            WHERE infant_hospital_number = ?1 AND archived = false ORDER BY id DESC LIMIT 1", nativeQuery = true)
    InfantPCRTest getLastPCR (String infantHospitalNumber);

    @Query(value = "SELECT * FROM public.pmtct_infant_pcr WHERE infant_hospital_number = ?1 AND pmtct_cycle_uuid = ?2 AND archived = false ORDER BY id DESC LIMIT 1", nativeQuery = true)
    ...
    @Query(value = "SELECT * FROM pmtct_infant_pcr WHERE visit_date =?2 and infant_hospital_number=?1 AND archived = false ORDER BY id DESC LIMIT 1", nativeQuery = true)
    InfantPCRTest getLatestInfantPCRInfo(String infantHospitalNo, LocalDate visitDate);

    @Query(value = "SELECT COUNT(*) > 0 FROM public.pmtct_infant_pcr WHERE infant_hospital_number = ?1 AND test_type = ?2 AND archived = false", nativeQuery = true)
    boolean existsByInfantHospitalNumberAndTestType(String infantHospitalNumber, String testType);

    @Query(value = "SELECT * FROM public.pmtct_infant_pcr WHERE infant_hospital_number = ?1 AND test_type = ?2 AND archived = false ORDER BY id DESC LIMIT 1", nativeQuery = true)
    InfantPCRTest findByInfantHospitalNumberAndTestType(String infantHospitalNumber, String testType);

    @Query(value = "SELECT * FROM public.pmtct_infant_pcr WHERE infant_hospital_number = ?1 AND visit_date = ?2 AND test_type = ?3 AND unique_uuid IS NULL AND archived = false ORDER BY id DESC LIMIT 1", nativeQuery = true)
    InfantPCRTest getLatestByHospitalNumberAndVisitDateAndTestType(String hospitalNumber, LocalDate visitDate, String testType);

    @Query(value = "SELECT * FROM public.pmtct_infant_pcr WHERE unique_uuid = CAST(?1 AS VARCHAR) AND infant_hospital_number = ?2 AND visit_date = ?3 AND archived = false ORDER BY id DESC LIMIT 1", nativeQuery = true)
