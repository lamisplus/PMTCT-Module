...
public interface InfantArvRepository extends CommonJpaRepository<InfantArv, String>
{
    ...
    @Query(value = "SELECT * FROM public.pmtct_infant_arv WHERE infant_hospital_number = ?1 AND visit_date = ?2 AND archived = false ORDER BY id DESC LIMIT 1", nativeQuery = true)
    InfantArv getLatestByHospitalNumberAndVisitDate(String hospitalNumber, LocalDate visitDate);

    @Query(value = "SELECT * FROM public.pmtct_infant_arv WHERE mother_patient_uuid = CAST(?1 AS VARCHAR) AND infant_hospital_number = ?2 AND visit_date = ?3 AND unique_uuid IS NULL AND archived = false ORDER BY id DESC LIMIT 1", nativeQuery = true)
    ...
    @Query(value = "SELECT * FROM public.pmtct_infant_arv WHERE unique_uuid = CAST(?1 AS VARCHAR) AND infant_hospital_number = ?2 AND visit_date = ?3 AND archived = false ORDER BY id DESC LIMIT 1", nativeQuery = true)
